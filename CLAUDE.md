...
- **⚠️ Saturation des souscriptions market data** : chaque snapshot SOUSCRIT ses conids côté serveur pour toute la session gateway (persiste ENTRE les runs ; le re-login ne purge PAS). Au-delà de quelques milliers, les snapshots reviennent **sans prix** (tout-ou-rien par lot). Géré : `unsubscribe_all_marketdata()` appelé en début de cycle + tous les 10 symboles (`run_collector`). Le warm-up du snapshot est ADAPTATIF (arrêt sur complétude/plateau, plafond 30 essais) — le flux différé EUREX est lent à l'ouverture européenne (~6s/lot vs ~2s l'après-midi).
